...
* NEW settings page (under Tools) to toggle debug mode, plugin dependencies, dashboard widgets, editor metaboxes, admin menus, and more
...
* new way of handling custom post type nav items: instead of filtering the nav menu output and tacking our CPT's on, we're actually creating real nav menu items in the DB. Then you can go to Appearance->Menus and re-order that item like any other
...
* if show_in_nav_menus is true, then an new item will be available at the top of the "view all" tab of a given CPT. Adding that to the menu will create a custom nav menu item configured to display CPT archives
